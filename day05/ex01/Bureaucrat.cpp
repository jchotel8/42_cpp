...
Bureaucrat::Bureaucrat(std::string name)
{
	_name = name;
    _grade = 150;
    std::cout << "[" << _name << ", "<< _grade << "] is hired!" << std::endl;
}
...
{
	std::cout << "[" << _name << ", "<< _grade << "] is fired, you won't notice" << std::endl;
	return ;
}
...
void    Bureaucrat::signForm(Form *f)
{
  ...
  {
    std::cout << "[" << _name << ", " << _grade << 
    "] I should pass it over to my superior, he'll give it to his boss and so on." << std::endl;
    std::cout << "[" << _name << ", " << _grade << "] can't sign ";
    f->print();
